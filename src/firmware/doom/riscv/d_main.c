...
void D_Display (void)
{
    ...
    do
    {
        ...
        {
            // Save pointer to the buffer we just rendered the wipe into
            byte *prev_scr = screens[0];
            I_FinishUpdate ();                  // page flip or blit buffer
            // Copy wipe state to the new draw buffer so both buffers
            // stay in sync (wipe does incremental updates)
            memcpy(screens[0], prev_scr, SCREENWIDTH * SCREENHEIGHT);
        }
    } while (!done);
}
...
void D_PageDrawer (void)
{
    ...
    /* Clear top/bottom margins and center 320x200 patch */
    memset(screens[0], 0, SCREENWIDTH * yoff);
    memset(screens[0] + SCREENWIDTH * (yoff + 200), 0, SCREENWIDTH * yoff);
    V_DrawPatch (0, yoff, 0, W_CacheLumpName(pagename, PU_CACHE));
}
